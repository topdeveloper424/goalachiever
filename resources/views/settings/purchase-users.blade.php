@extends('layout.app')
@section('title')
    User List (Real Estate Purchase Closing Cost)
@endsection
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-md-6">
                                <h4 class="card-title badge badge-danger badge-pill">Users Table ( Real Estate Purchase Closing Cost )</h4>
                            </div>
                            <div class="col-md-6">
                                    ...
                                    <div role="tabpanel" class="tab-pane @if($loop->iteration == 1) active @endif" id="tab{{$goal_type->id}}" aria-expanded="true" aria-labelledby="base-tab{{$goal_type->id}}">
                                        <table class="table table-striped table-bordered">
                                            ...
                                            @foreach($users as $user)
                                                <?php
                                                $goal = \App\PurchaseGoal::where('user',$user->id)->where('type',$goal_type->id)->get();
                                                $created = null;
                                                if (count($goal) > 0){
...
@section('script')
    ...
    <script>
        ...
        function gotoPage(id,type) {
            window.open("purchase?id="+id+"&type="+type);
        }
