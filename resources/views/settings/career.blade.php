...
@section('content')
    ...
    <div class="content-wrapper"><!--Statistics cards Starts-->
        <form action="{{route('update_career')}}" method="post">
            @csrf
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-md-5">
                                <h4 class="card-title" id="basic-layout-colored-form-control">Career Goal</h4>
                            </div>
                            <div class="col-md-5">
                                <button type="button" class="btn btn-danger gradient-pomegranate round mr-4" style="float: right" onclick="cashAlert({{$career->id}})"><i class="ft-alert-circle mr-1"></i>Cash Alert</button>
                                <button type="button" class="btn btn-primary gradient-crystal-clear round mr-4" style="float: right" onclick="creditAlert({{$career->id}})"><i class="ft-credit-card mr-1"></i>Credit Alert</button>
                            </div>
                            <div class="col-md-2">
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input" name="goal_achieved" id="goal_achieved" @if($career->goal_achieved == true) checked @endif>
                    ...
                    <input type="hidden" name="id" id="goal_id" value="{{$career->id}}">
                    <input type="hidden" name="type" id="type" value="{{$career->type}}">
                            ...
                            <div class="col-md-7">
                                    ...
                                    <div class="col-md-12" style="display: inline-flex">
                                        ...
                                        <select class="form-control grad-right"  style="color: black;" name="type" required>
                                            @foreach($goal_types as $goal_type)
                                                <option value="{{$goal_type->id}}" @if($career->type == $goal_type->id) selected @endif>{{$goal_type->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12" style="display: inline-flex">
                                        ...
                                        <input type="date" class="form-control grad-right" style="color: black;" value="{{$career->start_date}}" id="start_date" name="start_date" onchange="changeStartDate()" required>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12" style="display: inline-flex">
                                ...
                                <div class="row">
                                    <div class="col-md-12" style="display: inline-flex">
                                        ...
                                        <input type="number" class="form-control grad-right" style="color: black;" value="{{$career->time_frame}}" name="time_frame" id="time_frame" required>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12" style="display: inline-flex">
                                        <label class="form-control grad-left" style="color: white">Goal Status</label>
                                        <select class="form-control @if($career->status == true) grad-achieved @else grad-pending @endif" style="color: black;" name="status" id="status" required onchange="statusChange()">
                                            <option value="1" @if($career->status == true) selected @endif>Achieved</option>
                                            <option value="0" @if($career->status == false) selected @endif>Pending</option>
                                        </select>
                                    </div>
                                </div>
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td class="grad-right-table">
                                                        <input type="number" class="form-control grad-right-table" name="family" id="family" value="{{$career->family}}" onchange="drawChart()">
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="grad-left-table">Desire</td>
                                                    <td class="grad-right-table">
                                                        <input type="number" class="form-control grad-right-table" name="desire" id="desire" value="{{$career->desire}}" onchange="drawChart()">
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="grad-left-table" >Make Money</td>
                                                    <td class="grad-right-table">
                                                        <input type="number" class="form-control grad-right-table" name="make_money" id="make_money" value="{{$career->make_money}}" onchange="drawChart()">
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="grad-left-table">Other</td>
                                                    <td class="grad-right-table">
                                                        <input type="number" class="form-control grad-right-table" name="other" id="other" value="{{$career->other}}" onchange="drawChart()">
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
    ...
    <input type="hidden" id="curID" name="goal" value="{{$career->id}}">
...
// Chart Options
            ...
            var makeMoney = $("#make_money").val();
            var other = $("#other").val();

            var sum = Number(family) + Number(desire) + Number(makeMoney) + Number(other);
            var familyPercent = "Family : "+(Number(family)*100 / sum).toFixed(0)+"%";
            var desirePercent = "Desire : "+(Number(desire)*100 / sum).toFixed(0)+"%";
            var makeMoneyPercent = "Make Money : "+(Number(makeMoney)*100 / sum).toFixed(0)+"%";
            var otherPercent = "Other : "+(Number(other)*100 / sum).toFixed(0)+"%";


// Chart Data
            var chartData = {
                labels: [familyPercent, desirePercent, makeMoneyPercent, otherPercent],
                datasets: [{
                    label: "My First dataset",
                    data: [family, desire, makeMoney, other],
