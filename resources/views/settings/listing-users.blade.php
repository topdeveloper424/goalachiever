@extends('layout.app')
@section('title')
    User List (Real Estate Listing Closing Cost)
@endsection
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-md-6">
                                <h4 class="card-title badge badge-danger badge-pill">Users Table ( Real Estate Listing Closing Cost )</h4>
                            </div>
                            <div class="col-md-6">
                                    ...
                                    <div role="tabpanel" class="tab-pane @if($loop->iteration == 1) active @endif" id="tab{{$goal_type->id}}" aria-expanded="true" aria-labelledby="base-tab{{$goal_type->id}}">
                                        <table class="table table-striped table-bordered">
                                            ...
                                            @foreach($users as $user)
                                                <?php
                                                $goal = \App\ListingGoal::where('user',$user->id)->where('type',$goal_type->id)->get();
                                                $created = null;
                                                if (count($goal) > 0){
...
@section('script')
    ...
    <script>
        ...
        function gotoPage(id,type) {
            window.open("listing?id="+id+"&type="+type);
        }
