...
@section('content')
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-md-5">
                                <h4 class="card-title" id="basic-layout-colored-form-control">Personal Goal</h4>
                            </div>
                            <div class="col-md-5">
                                <button type="button" class="btn btn-danger gradient-pomegranate round mr-4" style="float: right" onclick="cashAlert({{$personal->id}})"><i class="ft-alert-circle mr-1"></i>Cash Alert</button>
                                <button type="button" class="btn btn-primary gradient-crystal-clear round mr-4" style="float: right" onclick="creditAlert({{$personal->id}})"><i class="ft-credit-card mr-1"></i>Credit Alert</button>
                            </div>
                            <div class="col-md-2">
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input" name="goal_achieved" id="goal_achieved" @if($personal->goal_achieved == true) checked @endif>
                    ...
                    <input type="hidden" name="id" id="goal_id" value="{{$personal->id}}">
                    <input type="hidden" name="type" id="type" value="{{$personal->type}}">
                            ...
                            <div class="col-md-7">
                                    ...
                                    <div class="col-md-12" style="display: inline-flex">
                                        ...
                                        <select class="form-control grad-right"  style="color: black;" name="type" required>
                                            @foreach($goal_types as $goal_type)
                                                <option value="{{$goal_type->id}}" @if($personal->type == $goal_type->id) selected @endif>{{$goal_type->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12" style="display: inline-flex">
                                        ...
                                        <input type="date" class="form-control grad-right" style="color: black;" value="{{$personal->start_date}}" id="start_date" name="start_date" onchange="changeStartDate()" required>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12" style="display: inline-flex">
                                ...
                                <div class="row">
                                    <div class="col-md-12" style="display: inline-flex">
                                        <label class="form-control grad-left" style="color: white;">Goal Time Frame(Days)</label>
                                        <input type="number" class="form-control grad-right" style="color: black;" value="{{$personal->time_frame}}" name="time_frame" id="time_frame" required>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12" style="display: inline-flex">
                                        <label class="form-control grad-left" style="color: white">Goal Status</label>
                                        <select class="form-control @if($personal->status == 1) grad-achieved @else grad-pending @endif" style="color: black;" name="status" id="status" required onchange="statusChange()">
                                            <option value="1" @if($personal->status == 1) selected @endif>Achieved</option>
                                            <option value="0" @if($personal->status == 0) selected @endif>Pending</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="row mt-4">
                                    <div class="col-md-12">
                                        <table class="table table-bordered">
                                            <thead>
                                            <tr>
                                                <th class="grad-left" width="70%">Motivation Type</th>
                                                <th class="grad-left" width="30%">% Value</th>
                                            </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td class="grad-right-table">
                                                        <input type="number" class="form-control grad-right-table" name="family" id="family" value="{{$personal->family}}" onchange="drawChart()">
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="grad-left-table">Desire</td>
                                                    <td class="grad-right-table">
                                                        <input type="number" class="form-control grad-right-table" name="desire" id="desire" value="{{$personal->desire}}" onchange="drawChart()">
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="grad-left-table" >Health</td>
                                                    <td class="grad-right-table">
                                                        <input type="number" class="form-control grad-right-table" name="health" id="health" value="{{$personal->health}}" onchange="drawChart()">
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="grad-left-table">Other</td>
                                                    <td class="grad-right-table">
                                                        <input type="number" class="form-control grad-right-table" name="other" id="other" value="{{$personal->other}}" onchange="drawChart()">
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
    ...
    <input type="hidden" id="curID" name="goal" value="{{$personal->id}}">
